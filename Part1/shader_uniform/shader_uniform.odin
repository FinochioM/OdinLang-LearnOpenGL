...
main :: proc() {
    // Read shader files
    vertex, ok_vertex := os.read_entire_file("Part1/shader_uniform/vertex.vert")
    if !ok_vertex {
    ...
    fragment, ok_fragment := os.read_entire_file("Part1/shader_uniform/fragment.frag")
    if !ok_fragment {
    ...
    gl.BufferData(gl.ARRAY_BUFFER, size_of(vertices), &vertices, gl.STATIC_DRAW) // size_of([9]f32) -> 9 * size_of(f32) -> 9 * 4 = 36 bytes
    ...
    for (!glfw.WindowShouldClose(_window)) {
        ...
        greenValue : f64 = (math.sin(timeValue) / 2.0) + 0.5
        vertexColorLocation : i32 = gl.GetUniformLocation(shaderProgram, "ourColor")

        gl.Uniform4f(vertexColorLocation, 0.0, f32(greenValue), 0.0, 1.0)
